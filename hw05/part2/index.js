...
var democratStateSorted = democratState.slice(0);
democratStateSorted.sort();
...
var repStateSorted = repState.slice(0);
repStateSorted.sort();
    ...
    d3.select("body").select(".chart").selectAll("div")
        ...
        .transition()
            ...
            .style('width',function(d) { 
                if (party == "d") {
                    var a = democratState.indexOf(d);
                    return (democratNumber[a] * 2.4) + "px";
                } else {
                    var a = repState.indexOf(d);
                    return (repNum[a] * 2.4) + "px";
                }
            }) 
        ...
        .text( function(d,i) { 
            if (party == "d") {
                var a = democratState.indexOf(d);
                return democratNumber[a] + " votes in " + d; 
            } else {
                var a = repState.indexOf(d);
                return repNum[a] + " votes in " + d; 
            }
        })
        .style("opacity",1);
};
